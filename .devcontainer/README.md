...
* prioritize layers from "won't change at all" down to "might change regularly" to decrease build time
if anything changed
...
Create a personal access token with at least `read_registry` permission [here](https://gitlab-intern.ait.ac.at/-/user_settings/personal_access_tokens). Then login via

```sh
echo "<TOKEN>" | docker login gitlab-intern.ait.ac.at:5010 -u <email> --password-stdin
```
